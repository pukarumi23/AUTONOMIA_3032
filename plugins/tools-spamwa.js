...
if (!nomor) return conn.reply(m.chat, `${emoji} Por favor, ingrese un número para realizar spam.`, m, rcanal);

if (!pesan) return conn.reply(m.chat, `${emoji} Uso Correcto:\n\n> ${emoji2} #spamwa numero|texto|cantidad`, m, rcanal);

if (jumlah && isNaN(jumlah)) return conn.reply(m.chat, `${emoji2} La cantidad deve ser un numero.`, m, rcanal);
