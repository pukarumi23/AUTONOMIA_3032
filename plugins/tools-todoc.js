...
if (!m.quoted) return conn.reply(m.chat,  `${emoji} Etiqueta el *Video o Audio* que desea convertir en documento.`, m, rcanal)
if(!text) return conn.reply(m.chat, `${emoji2} Ingresa el nombre para guardar el documento.`, m)
if (!/audio|video/.test(mime)) return conn.reply(m.chat,  `${emoji} Etiqueta el *Video o Audio* que desea convertir en documento.`, m, rcanal)
